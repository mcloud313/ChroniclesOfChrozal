...
async def cmd_teleport(character: 'Character', world: 'World', args_str: str) -> bool:
    ...
    await cmd_look(character, world, "")
    return True


async def cmd_roomstat(character: 'Character', world: 'World', args_str: str) -> bool:
    """Admin: Shows detailed stats for a room. Usage: @roomstat [room_id]"""
    room_to_stat = None
    if not args_str:
...
async def cmd_examine(character: 'Character', world: 'World', args_str: str) -> bool:
    """Admin: Shows debug info about world objects. Usage: @examine <type> <id>"""
    parts = args_str.lower().split(" ", 1)
    if len(parts) != 2 or not parts[1].isdigit():
        await character.send("Usage: @examine <char|mob|item_template|room> <id>")
        return True

    obj_type, identifier = parts[0], int(parts[1])
    output = [f"--- Examining {obj_type} '{identifier}' ---"]
    found = False

    if obj_type == "char":
        target_char = world.get_active_character(identifier)
        if target_char:
        ...
        target_mob = next((m for r in world.rooms.values() for m in r.mobs if m.instance_id == identifier), None)
        if target_mob:
            output.append(f" Template ID: {target_mob.template_id}")
            output.append(f" HP: {target_mob.hp}/{target_mob.max_hp}")
            output.append(f" Target: {getattr(target_mob.target, 'name', 'None')}")
            found = True
    elif obj_type == "item_template":
        template_data = world.get_item_template(identifier)
        if template_data:
            output.append(json.dumps(template_data, indent=2))
            found = True
...
async def cmd_setstat(character: 'Character', world: 'World', args_str: str) -> bool:
